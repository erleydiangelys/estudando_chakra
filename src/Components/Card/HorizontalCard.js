...
function HorizontalCard({card}) {
    ...
    return (
        <Flex direction={{sm:'column', md: 'row'}} position='relative' borderRadius='8px' bgColor={useColorModeValue('gray.100', 'black')}>
            ...
            <Image src={data.imgURL} alt={name} w='100%' h='300px' objectFit='cover' borderLeftRadius={{sm:'0', md: '8'}} borderTopRadius={{sm:'8', md: '0'}}/>

            <Box p='16px' spacing='16px'>
                <Box align='end'>
                    <Text variant='subtitle'>{data.dataPost}</Text>
            </Box>

            <Heading size='md'  mt='4' align='center'>{data.title}</Heading>

            <Text overflowY='auto' mt='4' mb='8' css={customScrollbar}>{data.resumo}</Text>
